...
const PROJECT_LIST_FIELDS = 'id, tool_id, name, created_at, updated_at';

function listProjects(userId, toolId) {
  return db
    .prepare(
      `SELECT ${PROJECT_LIST_FIELDS} FROM tool_projects
       WHERE user_id = ? AND tool_id = ? ORDER BY updated_at DESC`
    )
    .all(userId, toolId);
}
